    ...
    !function loop(RequestItems) {
      ...
      function fetch(err, data) {
        ...
        Object.keys(data.Responses).forEach(function(name) {
          ...
          data.Responses[name].forEach(function(attrs) {
            table.push(Attributes.prototype.parse(attrs))
          })
        })

        if (data.UnprocessedKeys && data.UnprocessedKeys.keys && data.UnprocessedKeys.keys().length ) loop(data.UnprocessedKeys)
